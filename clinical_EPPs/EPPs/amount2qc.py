...
class Amount2QC:
    ...
    def get_artifacts(self):
        ...
        if not self.process.type.name == "CG002 - Qubit QC (Library Validation)":
            self.wrong_factor2 = self.check_udf_is_defined(
                self.correct_artifacts, "Volume (ul)"
            )
    ...
    def apply_calculations(self):
        ...
        if self.correct_artifacts:
            logging.info(
                "result_udf: Amount (ng), udf1: Concentration, operator: *, udf2: Volume (ul)"
            )
            for artifact in self.correct_artifacts:
                try:
                ...
                artifact.put()
                logging.info(
                    "Updated Amount (ng) to {0}.".format(artifact.udf["Amount (ng)"])
                )
...
def main(lims, args):
    ...
    if not A2QC.amount_treshold:
        abstract = (
            'Unable to set QC-flaggs. "Minimum requiered amount (ng)" has not been set.'
        )
        sys.exit(abstract)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", dest="log", help="Log file for runtime info and errors.")
