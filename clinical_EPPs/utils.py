...
def get_latest_artifact(lims: Lims, sample_id: str, process_type: list) -> Artifact:
    ...
    if not artifacts:
        raise MissingArtifactError(
            message=f"Could not find a artifact with {sample_id} generated by process: {' ,'.join(process_type)}"
        )
...
def get_lims_log_file(lims, file_id: "str") -> pathlib.Path:
    ...
    if files:
        ...
        file_path = pathlib.Path(files[0].content_location.split(something)[1])
    else:
        file_path = file_id
